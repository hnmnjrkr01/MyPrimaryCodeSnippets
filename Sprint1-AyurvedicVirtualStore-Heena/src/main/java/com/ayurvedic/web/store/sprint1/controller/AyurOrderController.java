...
//	private static final HttpStatus HttpStatus = null;
	...
	*/
	...
	@GetMapping(path="/totalCost/{cost}")
	public ResponseEntity<Double> totalCost(@PathVariable("cost") double cost){
		double totalCost = orderService.totalCost(cost);
		return new ResponseEntity<Double>(totalCost,new HttpHeaders(),HttpStatus.OK);
	}
