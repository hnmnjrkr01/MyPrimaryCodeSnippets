...
public class AyurOrderServiceImpl implements IAyurOrderService{
	...
	public double totalCost(double cost) {
		...
		List<AyurOrder> orderTotalCost = orderRepository.findAll();
		AyurOrder orderCost = new AyurOrder();
		Iterator it = orderTotalCost.listIterator();
		double totalCost=0;
		
		while(it.hasNext()){
			orderCost = (AyurOrder)it.next();
			if(orderCost.getCost()==cost){
				totalCost = orderCost.getTotalCost();
				return totalCost;
			}
			throw new OrderNotPlacedException("No AyurOrder found with the provided cost : "+cost);
		}
		return totalCost;
	}
