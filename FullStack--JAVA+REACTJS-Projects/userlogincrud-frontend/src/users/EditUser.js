...
export default function EditUser() {
  ...
  const onSubmit = async (e)=>{
    e.preventDefault();
    await axios.put(`http://localhost:8080/UserCRUDLogin/updateUser/${userId}`,user);
    navigate("/");
  }
   
  const loadUser = async()=>{
    const result = await axios.get(`http://localhost:8080/UserCRUDLogin/updateUser/${userId}`);
    setUser(result.data);
  }
